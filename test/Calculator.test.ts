...
function updateData(): void {
  calculator.updateData(2.05, 88, 14.5, 71, 41, (5 * 60), 20);
}
...
test("Should have climbingTime 509 for climbDistance 2.05, climbingSpeed 14.5", () => {
  updateData();
  expect(calculator.getClimbingTime()).toBe(508.9655172413792);
});

test("Should have totalClimbingTime 51405", () => {
  updateData();
  expect(calculator.getTotalClimbingTime()).toBe(51405.5172413793);
});

test("Should have descendingTime 180", () => {
  updateData();
  expect(calculator.getDescendingTime()).toBe(179.99999999999997);
});

test("Should have totalDescendingTime 18180", () => {
  updateData();
  expect(calculator.getTotalDescendingTime()).toBe(18179.999999999996);
});

test("Should have totalTime 69885", () => {
  updateData();
  expect(calculator.getTotalTime()).toBe(69885.51724137929);
});

test("Should have totalRestStopTime 6000", () => {
  updateData();
  expect(calculator.getTotalRestStopTime()).toBe(6000);
});
